...
class upload
{
	...
	private $_filePath = '../../../../upload/';              //�ϴ��ļ�����Ŀ¼

	/**
	 * ��ȡ����
	 */
	public function tk(){
		$file['name'] = $_GET['name'];                  //文件名
		$file['size'] = $_GET['size'];                  //文件大小
		$file['token'] = md5(json_encode($file['name'] . $file['size']));
		//$file['token'] = uniqid(rand(time()));
        //获取上传后文件信息
		//�ж��Ƿ���ڸ�������Ϣ
		if(! file_exists($this->_tokenPath . $file['token'] . '.token')){
            ...
            $file['servername'] = $file['token'] .'.'. $pathInfo['extension'];
            //将文件放入session
            $_SESSION['file_name']['servername'][] = $file['token'] .'.'. $pathInfo['extension']; //上传后文件名
            $_SESSION['file_name']['name'][] = $file['name'];
